...
public class PojoToVo {
    ...
    ProductVO s=new ProductVO();
        s.setImageHost(PropertiesUTils.getValues("imageHost"));
        s.setId(p.getId());
        s.setCategory_id(p.getCategory_id());
        s.setName(p.getName());
        s.setSubtitle(p.getSubtitle());
        s.setSub_images(p.getSub_images());
        s.setMain_image(p.getMain_image());
        s.setDetail(p.getDetail());
        s.setPrice(p.getPrice());
        s.setStock(p.getStock());
        s.setStatus(p.getStatus());
        s.setIs_new(p.getIs_new());
        s.setIs_hot(p.getIs_hot());
        s.setIs_banner(p.getIs_banner());
        s.setCreate_time(p.getCreate_time());
        s.setUpdate_time(p.getUpdate_time());
        return s;
    }
    ...
    public static CartProductVO getOne(Cart cart, Product p){
        ...
        if(p != null){
            ...
            cartProductVO.setMain_image(p.getMain_image());
        ...
        if(cart.getQuantity()<= p.getStock()){
            ...
            cartProductVO.setLimitQuantity(Consts.Cart.LIMITQUANTITYSUCCESS);
        }else{
            count = p.getStock();
            cartProductVO.setLimitQuantity(Consts.Cart.LIMITQUANTITYDEFEAT);
        }
    ...
    /*商品详情和支付宝商品类转换*/
    public static PGoodsDetail getNewPay(Orders_item orderItem){
    ...
    /*获取一个BizContent对象*/
    public static BizContent getBizContent(Orders order, List<Orders_item> orderItems){
        ...
        for (Orders_item orderItem : orderItems) {
